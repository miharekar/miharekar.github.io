...
You might be wondering if `:do_not_deliver` is some special Rails magic symbol. It's not. You could set it to `:please_deliver` or `:foobar` and it would still **not** be delivered. The reason is that if the `response_body` is set to _anything_, the email will not be sent. So how does that work?
